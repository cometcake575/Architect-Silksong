...
public class FloatConfigType<T>(
    ...
    ) : ConfigType<T, FloatConfigValue<T>>(name, id, action) where T : ScriptBlock
{
    ...
    {
        return new FloatConfigValue<T>(this, Convert.ToSingle(data, CultureInfo.InvariantCulture));
    }
}

public class FloatConfigValue<T>(FloatConfigType<T> type, float value) 
    : ConfigValue<FloatConfigType<T>>(type)
    where T : ScriptBlock
{
