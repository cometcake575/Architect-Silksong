...
public class ChoiceConfigType<T>(
    ...
    ) : ConfigType<T, ChoiceConfigValue<T>>(name, id, action) where T : ScriptBlock
{
    ...
    {
        return new ChoiceConfigValue<T>(this, Convert.ToInt32(data, CultureInfo.InvariantCulture));
    }
...
public class ChoiceConfigValue<T>(ChoiceConfigType<T> type, int value) 
    : ConfigValue<ChoiceConfigType<T>>(type) where T : ScriptBlock
{
